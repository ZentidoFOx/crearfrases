...
class UserController
{
    ...
    public static function changePassword(): void
    {
        ...
        // ⚠️ CORRECCIÓN: Usar findByUsername porque la tabla no tiene 'email'
        // Verify current password
        $fullUser = User::findByUsername($user['username']);
        
        if (!$fullUser) {
            Response::error('Usuario no encontrado', 'USER_NOT_FOUND', null, 404);
        }
     ...
     */
    public static function getWordPressCredentials(int $websiteId): void
    {
        $user = $_REQUEST['auth_user'] ?? null;
        ...
        try {
            ...
            $credentials = WordPressCredentials::getByUserAndWebsite($user['id'], $websiteId);
            ...
            // Return the actual decrypted password
            Response::success([
                'username' => $credentials['username'],
                'app_password' => $credentials['app_password'], // Real decrypted password
                'is_configured' => true,
                'test_status' => $credentials['test_status'] ?? 'pending',
                'last_tested' => $credentials['last_tested'] ?? null
            ]);

        } catch (\Exception $e) {
            error_log('Get WordPress credentials error: ' . $e->getMessage());
            Response::serverError('Error al obtener credenciales');
        }
    }
     ...
     */
    public static function saveWordPressCredentials(int $websiteId): void
    {
        $user = $_REQUEST['auth_user'] ?? null;
        ...
        if (!$data || !isset($data['username']) || !isset($data['app_password'])) {
            Response::error('Usuario y contraseña de aplicación son requeridos', 'INVALID_DATA', null, 400);
        }
        ...
        if (empty($username) || empty($appPassword)) {
            Response::error('Usuario y contraseña de aplicación no pueden estar vacíos', 'INVALID_DATA', null, 400);
        }

        try {
            $success = WordPressCredentials::saveCredentials($user['id'], $websiteId, $username, $appPassword);

            if (!$success) {
                Response::serverError('Error al guardar las credenciales');
            }

            Response::success(null, 'Credenciales guardadas exitosamente', 201);

        } catch (\Exception $e) {
            error_log('Save WordPress credentials error: ' . $e->getMessage());
            Response::serverError('Error al guardar credenciales');
        }
    }
     ...
     */
    public static function testWordPressConnection(int $websiteId): void
    {
        $user = $_REQUEST['auth_user'] ?? null;
        ...
        try {
            $result = WordPressCredentials::testConnection($user['id'], $websiteId, $username, $appPassword);

            if ($result['success']) {
                Response::success($result['user_data'] ?? null, $result['message']);
            } else {
                Response::error($result['error'], 'CONNECTION_FAILED', null, 400);
            }

        } catch (\Exception $e) {
            error_log('Test WordPress connection error: ' . $e->getMessage());
